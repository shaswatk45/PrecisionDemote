...
function HoverCard({ node, x, y }) {
  ...
  return (
    <motion.div
      ...
      className="absolute z-30 w-64 rounded-sm border bg-[#0d0d0d] p-4 shadow-nv-glow pointer-events-none font-sans uppercase"
      style={{ left: x, top: y, borderColor: `${col}55` }}
    >
      <div className="flex items-center gap-2">
        <span className="font-mono font-bold text-white text-xs normal-case">{node.name}</span>
        <span
          className="ml-auto font-mono text-[9px] px-2 py-0.5 rounded-sm border font-bold uppercase tracking-wider"
      ...
      <div className="mt-3 space-y-2">
        <div>
          <div className="flex justify-between text-[9px] text-mute font-bold tracking-widest mb-1">
            <span>safety score</span>
            <span className="font-mono" style={{ color: col }}>{score}/100</span>
          </div>
          <div className="h-1.5 rounded-sm bg-white/10 overflow-hidden">
            <motion.div
        ...
        <div className="flex justify-between text-[10px] tracking-wide">
          <span className="text-mute font-bold">est. rel. error</span>
          <span className="font-mono text-white font-bold lowercase">{err}</span>
        </div>
        <div className="flex justify-between text-[10px] tracking-wide">
          <span className="text-mute font-bold">depth · fan-in</span>
          <span className="font-mono text-white font-bold">{node.depth} · {node.dependencyCount}</span>
        </div>
        ...
        <motion.div
          ...
          className="text-[10px] pt-1 border-t border-line font-bold tracking-wide"
          style={{ color: col }}
        >
...
export default function CodeEditor({
  ...
  return (
    <div ref={containerRef} className="relative overflow-hidden rounded-sm" style={{ height }}>
      <Editor
        height="100%"
        language="cpp"
        ...
        loading={<div className="p-6 text-mute text-xs uppercase tracking-wider font-sans font-bold">Loading editor…</div>}
      />
