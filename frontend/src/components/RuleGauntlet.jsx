...
export default function RuleGauntlet({ node, thresholds = { maxDepth: 3, maxFanIn: 5 }, stepMs = 480 }) {
  ...
  return (
    <div className="space-y-1.5 font-sans uppercase">
      {RULES.map((rule, i) => {
        ...
        return (
          <motion.div
            key={rule.key}
            className={`flex items-center gap-2.5 rounded-sm px-2.5 py-1.5 border ${fail ? 'pd-redline' : ''}`}
            animate={{
              borderColor: checking ? 'rgba(118,185,0,0.5)' : fail ? `${ROSE}66` : pass ? `${TEAL}33` : 'rgba(255,255,255,0.06)',
              backgroundColor: fail ? 'rgba(244,63,94,0.10)' : pass ? 'rgba(20,184,166,0.06)' : 'rgba(255,255,255,0.02)',
              opacity: skipped ? 0.3 : 1,
              scale: fail ? [1, 1.02, 1] : 1,
            }}
          ...
          >
            ...
            >
              <Icon className="w-3.5 h-3.5" style={{ color: checking ? '#76b900' : col }} />
            </motion.span>

            <span className="text-xs font-bold" style={{ color: skipped ? '#4b5563' : resolved ? '#e2e8f0' : '#6b7280' }}>
              {rule.name}
            </span>
            <span className="ml-auto font-mono text-[10px] text-mute normal-case">
            ...
            <span className="w-4 flex justify-center">
              ...
              {checking && <span className="w-1.5 h-1.5 rounded-full bg-nv animate-pulse" />}
            </span>
          </motion.div>
        )
      })}
      ...
      {done && (
        <motion.div
          ...
          className="rounded-sm px-3 py-2 border text-xs font-bold flex items-center gap-2"
          style={redirected
        ...
        >
          {redirected ? (
            <>
              ...
              Redirected <ArrowRight className="w-3 h-3" /> <span className="font-mono lowercase font-normal">__bf16</span>
              <span className="text-[10px] opacity-80 ml-1">same size, wider range</span>
            </>
          ) : failed ? (
            <>Kept at <span className="font-mono lowercase font-normal">float</span> — blocked by {RULES[failIndex].name.toLowerCase()}</>
          ) : (
            <>All 6 rules pass <ArrowRight className="w-3 h-3" /> <span className="font-mono lowercase font-normal">__fp16</span></>
          )}
        </motion.div>
      )}
    </div>
