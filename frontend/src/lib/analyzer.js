export const DEFAULT_THRESHOLDS = Object.freeze({ maxDepth: 3, maxFanIn: 5 });

export const FP16_MAX = 65504.0;
export const FP16_UNIT_RND = 4.8828125e-4; // 2^-11
export const BF16_UNIT_RND = 3.90625e-3; // 2^-8

export function maxLiteralMagnitude(expr) {
  if (!expr) return -1;
...
export function stripComments(sourceCode) {
  return sourceCode
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/\/\/.*$/gm, '');
}

export function splitFunctions(sourceCode, filename) {
...
export function computeBlockReason(node, thresholds) {
...
export function fallbackAnalysis(sourceCode, filename, thresholds = DEFAULT_THRESHOLDS) {
...
export function legacyBlockReason(node) {
...
export function computeMetrics(analysis) {
  ...
  return {
    ...
    estimatedSpeedup,
    rooflineBound: narrowedCount > 0 ? (total > 15 ? 'Memory-bound (BW Limited)' : 'Compute-bound (ALU Limited)') : 'N/A',
    simdHint: narrowedCount > 0 ? 'Consider AVX-512 / Neon FP16 auto-vectorization for 2x throughput' : 'No SIMD benefit',
    dataFlowComplexity: Math.floor(allNodes.length * 1.5),
